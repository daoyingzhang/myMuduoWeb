...
bool HttpRequest::parse(Buffer& buff) {
    ...
    while(buff.readableBytes() && state_ != FINISH) {
        ...
        // LOG_DEBUG<<"find: "<<
        if(lineend==NULL) lineend = buff.beginWrite();
...
void HttpRequest::ParseHeader_(const string& line) {
    ...
    if(regex_match(line, Match, patten)) {
        ...
        LOG_DEBUG<<"state change:"<<line.size();
        state_ = BODY;
    }
}
...
void HttpRequest::ParsePost_() {
    // LOG_DEBUG<<"ParsePost_";
    if(method_ == "POST" && header_["Content-Type"] == "application/x-www-form-urlencoded") {
        // LOG_DEBUG<<"ParsePost_ IN";
...
bool HttpRequest::UserVerify(const string &name, const string &pwd, bool isLogin) {
    // LOG_DEBUG<<"Verify name:%d pwd:%d", name.size(), pwd.size();
    if(name == "" || pwd == "") { return false; }
    // LOG_DEBUG<<"Verify name: "<<name.c_str()<< "   pwd: " << pwd.c_str();
    //这个里边直接自定义一个删除器，所以没必要在写一个类了。
    std::shared_ptr<MysqlConn> sql=SqlConnPool::getInstance()->getConnection();
    // LOG_DEBUG<<"this sql";
    assert(sql);
    ...
    if(!isLogin && flag == true){
        ...
        LOG_DEBUG<<"login " << std::string(order);
        if(!sql->query(order)) { 
