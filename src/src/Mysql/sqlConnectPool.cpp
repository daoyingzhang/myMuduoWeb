...
SqlConnPool::~SqlConnPool() {
    LOG_DEBUG<<"begin to delete sqlpool";
    // 释放队列里管理的MySQL连接资源
    while (!connectionQueue_.empty())
    {
        MysqlConn *conn = connectionQueue_.front();
    ...
    while(true){
        ...
        while(connectionQueue_.size()>minSize_){
            ...
            if(conn->getAliveTime()>maxIdleTime_){
                ...
                --currentSize_;
                cond_.notify_all();
                LOG_DEBUG<<"delete the connect of the sql whoes time exceed thd limit.";
            }else {
                break;
            }
        }
