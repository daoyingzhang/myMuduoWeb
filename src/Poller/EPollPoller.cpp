...
EPollPoller::EPollPoller(EventLoop *loop) :
        ...
        events_(kInitEventListSize) //存放时间描述符的位置大小是16
{
    if (epollfd_ < 0)
    {
        LOG_INFO << "epoll_create() error:" << errno;
    }
}
...
Timestamp EPollPoller::poll(int timeoutMs, ChannelList *activeChannels)
{
    ...
    //numEvents表示events_中已经被存储的事件的数量
...
void EPollPoller::update(int operation, Channel *channel)
{
    ...
    /**
     * 下述操作 按照 “operation” 修改 epollfd_ （添加event还是删除），
     * 然后将套接字fd(这个应该是服务器端的)绑定到 epollfd_ 上，
     * 然后使用 epollfd_ 是 ::epoll_wait() 中监听感兴趣的事件(channel->events())。
     */
